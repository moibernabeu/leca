...
odir = '../outputs/LECA_proteomes_vircleaned/'
metabolismdir = '../outputs/metabolism_vircleaned/'
...
for (db in c('TOLDBA', 'TOLDBB', 'TOLDBC')) {
  ...
  mLECAori <- read.csv(paste0('../outputs/mLECA_origins/', db, '_stats.tsv'), sep = '\t')[, -c(23, 24)]
  ...
  innovations <- total_OGs[!(total_OGs %in% expan_OGs)]
