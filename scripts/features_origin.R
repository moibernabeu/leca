...
dat <- read.csv('../outputs/metabolism/donor_domain_KEGG_donors.tsv', sep = '\t')
...
write.table(features_summary %>%
              pivot_wider(names_from = donor, values_from = donor_prop, values_fill = 0),
            file = '../outputs/metabolism/donor_KEGG_donors_summary.tsv', sep = '\t', quote = FALSE, row.names = FALSE)
...
paths <- read.csv('../outputs/old_donors/metabolism_vircleaned/donor_domain_paths.tsv', sep = '\t') %>%
  filter(In.LECA == 'X')
paths <- paths$pathway

library(see)
ggplot(features_summary %>% filter(type == 'Cellular Processes', pathway %in% paths), aes(donor_prop, pathway, fill = donor)) +
  geom_col() +
  scale_fill_okabeito() +
  labs(title = 'Cellular Processes')

dat <- features_summary %>%
  filter(!(donor == 'Other' & donor_prop >= 0.15), !(donor %in% c('Other', 'Viruses'))) %>%
  group_by(type, subtype, pathway) %>%
  # mutate(new_total = sum(donor_prop)) %>%
  # filter(new_total >= 0.85) %>%
  # mutate(new_prop = donor_prop / new_total) %>%
  # select(!c(new_total, donor_prop)) %>%
